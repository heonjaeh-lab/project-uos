...
function mkMarker(x,y,color,txt){
  return `<g transform="translate(${x},${y})"><circle r="10" fill="${color}" stroke="#fff" stroke-width="3"/>`+
    `<text y="3.4" text-anchor="middle" font-size="10" font-weight="800" fill="#fff" font-family="Gothic A1,sans-serif">${txt}</text></g>`;}
function renderMap(elId,routeIdx,opts){
  opts=opts||{};const detail=opts.detail!==false;
  const el=$(elId);const [mnx,mny,mxx,mxy]=DATA.bbox;
  const W=320,H=el.clientHeight||200;
  const px=lo=>(lo-mnx)/(mxx-mnx)*W, py=la=>H-(la-mny)/(mxy-mny)*H;
  let s=`<svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid slice" style="width:100%;height:100%;display:block"><rect width="${W}" height="${H}" fill="#EFEADD"/>`;
  // 공원(연한 초록) — 은은하게
  s+=`<ellipse cx="${W*.22}" cy="${H*.26}" rx="72" ry="46" fill="#DCEBC8" opacity=".6"/><ellipse cx="${W*.83}" cy="${H*.84}" rx="80" ry="52" fill="#DCEBC8" opacity=".6"/>`;
  // 배경 도로망 — detail에서만, 아주 옅고 얇게(경로가 돋보이도록)
  if(detail){for(const e of DATA.context.slice(0,150)){s+=`<polyline points="${e.map(c=>px(c[0])+','+py(c[1])).join(' ')}" fill="none" stroke="#E5DDCD" stroke-width="1.6" stroke-linecap="round"/>`;}}
  const rt=DATA.routes[routeIdx];
  const path=sg=>sg.line.map(c=>px(c[0])+','+py(c[1])).join(' ');
  // 경로: 굵은 흰 테두리 + 그늘색(햇볕→그늘). 얇은 도로 위에서 확실히 돋보이게.
  for(const sg of rt.segs)s+=`<polyline points="${path(sg)}" fill="none" stroke="#fff" stroke-width="12" stroke-linecap="round" stroke-linejoin="round"/>`;
  for(const sg of rt.segs)s+=`<polyline points="${path(sg)}" fill="none" stroke="${shadeColor(sg.shade)}" stroke-width="6.5" stroke-linecap="round" stroke-linejoin="round"/>`;
  // POI: 흰 바탕 + 컬러 링(또렷하게). detail에서만.
  if(detail){for(const p of rt.pois){s+=`<g transform="translate(${px(p.lon)},${py(p.lat)})"><circle r="7" fill="#fff"/><circle r="7" fill="none" stroke="${poiColor(p.type)}" stroke-width="3"/><circle r="2.8" fill="${poiColor(p.type)}"/></g>`;}}
  // 출발·도착 (펄스 제거 → 깔끔한 정적 마커)
  const first=rt.segs[0]?rt.segs[0].line[0]:DATA.origin;
  const last=rt.segs.length?rt.segs[rt.segs.length-1].line.slice(-1)[0]:DATA.dest;
  s+=mkMarker(px(first[0]),py(first[1]),'#35B36B','출');
  s+=mkMarker(px(last[0]),py(last[1]),'#F1592A','착');
  // 산책중: 현위치(정적, 애니메이션 없음)
  if(opts.pos&&rt.segs.length){const mid=rt.segs[Math.floor(rt.segs.length*0.42)].line[0];
    s+=`<g transform="translate(${px(mid[0])},${py(mid[1])})"><circle r="7.5" fill="#F1592A" stroke="#fff" stroke-width="3"/></g>`;}
  s+=`</svg>`;
  const avg=Math.round(rt.shade*100);
  el.innerHTML=s+`<div class="legend"><span style="width:9px;height:9px;border-radius:3px;background:#26A99B"></span>그늘 ${avg}%</div>`;
}
